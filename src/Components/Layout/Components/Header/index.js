...
// bind classname rồi trả lại một function cx chứa các classNames
...
function Header() {
    // Biến thay đổi giao diện actions ( login hay khong login )
    ...
    const handleMenuChange = (menuitem) => {
        switch (menuitem.type) {
            case 'language':
                ...
                // Xử lí gì đó bla bla bla
                break;
            default:
        }
    };

    const userLogin_Menu = [
        {
        ...
        <header className={cx('wrapper')}>
            <div className={cx('inner')}>
                ...
                <div className={cx('action')}>
                    {userlogin ? (
                        <>
                            {/* Icon ở trạng thái login rồi  */}
                            <Link to={`/upload`}>
                                <Button text leftIcon={<FontAwesomeIcon icon={faPlus} />}>
                                    Upload
                                </Button>
                            </Link>
                            <Tippy content="message-video" placement="bottom" delay={[0, 200]}>
                                <button>
                                    <MessageIcon className={cx('IconMessgage')} />
                                </button>
                            </Tippy>
                        ...
                        <Button rounded className={cx('customPrivate')}>
                        Get app
                        </Button> */}
                    ...
                    <Menu items={userlogin ? userLogin_Menu : MENU_ITEMS} onChange={handleMenuChange}>
                        {userlogin ? (
                            // Component : Imgae : dùng đi dùng lại ở mọi nơi
                            <Image
                                src="https://1.bp.blogspot.com/-1F0txd6CTXk/XyjEn1_2atI/AAAAAAAAiX0/2X1yKYprbxEAPvrNA4VoX_nuv8nawCSuQCNcBGAsYHQ/d/a5a84b393a0778e047f87c79478481e7.jpg"
                                className={cx('user-avatar')}
                                alt=""
                                // Custom riêng => tự truyền vào failback
                                // failback="https://static.fullstack.edu.vn/static/media/f8-icon.18cd71cfcfa33566a22b.png"
                            ></Image>
                        ) : (
